...
@export var dashing_time = 1.5
@export var dash_line_spawn_position = Vector2(0, 19)
@export var follow_player_while_dashing = false
...
func enter():
	...
	dash_duration_timer.wait_time = dashing_time
	follow_player = follow_player_while_dashing
	
	(parent.body_animation_player as AnimationPlayer).speed_scale = dash_speed_modifier / 2
	(parent.legs_animation_player as AnimationPlayer).speed_scale = dash_speed_modifier / 2
	
	is_dash_charging = true
	parent.is_following_player = false # stop following so that dash can charge
	move_speed = get_correct_move_speed() * dash_speed_modifier
